...
exports.get = function (structure, id, callback, includeIds) {
  var queryBuilder = queyBuilder.create();

  queryBuilder.select(structure, id, includeIds);
...
function addJoins(relationships, parentRelation, queryBuilder, includeIds) {
  ...
  while (key !== undefined) {
    queryBuilder.addRelation(relationships[key], parentRelation, includeIds);
    addJoins(relationships[key].resource.relationships, relationships[key], queryBuilder, includeIds);
    key = keys.pop();
  }
}
...
function formatJsonApiDataObj(data, type) {
  ...
  return data.map(function (item) {
    ...
    if (item.attributes.id !== undefined) {
      returnObj.data_id = item.attributes.id.toString();
    }
